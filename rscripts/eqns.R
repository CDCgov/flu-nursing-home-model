...
sir_generator <- odin::odin({
  ...
  # force of infection
  lambda_S_SR <- beta * (C_RR * infectious_resid + C_RS * infectious_staff) + import_rate
  ...
  lambda_S_HC <- beta * (C_SR * infectious_resid + C_SS * infectious_staff) + import_rate
  ...
  # baseline: assume no additional importation from community
  import_rate <- user(0)
